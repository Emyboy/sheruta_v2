...
const Eachval = ({
    ...
    const handleDelete = () => {
        ...
        axios(process.env.REACT_APP_BASE_URL + '/property-requests/' + val.id, {
            method: 'DELETE',
            headers: {
    ...
    return (
        <div className='col-lg-4 col-md-6' id={`request-${val.id}`}>
            {
                state.confirmDelete ? <div className="m-2 single-comment bg-dark  card shadow p-4 text-center">
                    ...
                    <div className="comment-details p-2">
                        ...
                        <hr className='mt-0' />
                        <Link className='text-theme' to={`/request/${val.uuid}/${val.users_permissions_user.id}`}>
                            <h2 style={{ fontSize: 0, height: 0 }}>{val.heading}</h2>
                        </Link>
                    ...
                    <div className='card-footer d-flex justify-content-between'>
                        <div>
                            ...
                            {
                                auth.user && auth.user.user.id === val.users_permissions_user.id ?
                                    <span><i onClick={() => setState({ ...state, confirmDelete: true })} className='fa fa-trash link text-theme ml-4'></i></span> : null
                            }
                        </div>
